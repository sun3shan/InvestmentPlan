...
def loadData(csv_file):
    try:
        data = pd.read_csv(csv_file, index_col = ['od_code'])
    except:
...
def getPkgNums(ownPrice, otherPrice, numPkg):
    ...
    for i in range(len(otherPrice)):
        otherNum.append(np.round(y[i]/w*numPkg, 6))
    return np.round(ownNum, 6), otherNum

if __name__ == '__main__':
#    raw_data = loadData('data.csv')
    raw_data = loadData('../data2/pricing_info.csv')
...
#    _pwd = 'leaderboard2/'
    ...
    for _dir in dirs:
        if '.csv' in _dir:
            continue
        for file in os.listdir(_pwd+_dir):
            if not '.csv' in file:
                continue
            f = open(_pwd+_dir+'/'+file)
            x = pd.read_csv(f)
            f.close()
